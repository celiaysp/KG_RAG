...
vector_retriever = vectorstore.as_retriever(search_type="similarity", search_kwargs={"k": RETRIEVER_K_VECTOR})
...
def clean_query(q): return re.sub(r'[^\w\s]', ' ', q)
def combine_documents(results): return deduplicate_docs(results["bm25"] + results["vetor"])
...
graph_embedder = LiteLLMEmbedder(model=f"ollama/{EMBED_MODEL}", api_base=f"{OLLAMA_BASE_URL}/v1")
...
@cl.on_chat_start
async def start():
    await cl.Message(content="Oi sou um assistente virtual da UFOPA, como posso ajudar?").send()
...
async def main(message: cl.Message):
    ...
    msg.content = "Verificando conexões e regras institucionais no Grafo..."
    await msg.update()
    
    try:

        resultado = await rag.query(message.content)
        

        graph_answer = getattr(resultado, 'response', getattr(resultado, 'answer', str(resultado)))
        
        if graph_answer and len(graph_answer.strip()) > 5:
            graph_doc = Document(
                page_content=f"[Contexto Estrutural - Grafo Institucional]: {graph_answer}",
                metadata={
                    "source": "FalkorDB / GraphRAG V1", 
                    "page": "N/A", 
                    "original_content": graph_answer
                }
            )
            unique_docs.append(graph_doc)
    except Exception as e:
        print(f"Busca no grafo ignorada ou sem resultados: {e}")
    ...
    reranked_docs = await loop.run_in_executor(None, rerank_docs, reranker, message.content, unique_docs)
    ...
    fontes = sorted(set(f"{d.metadata.get('source')} (pág {d.metadata.get('page', '?')})" for d in reranked_docs))
    response += "\n\n---\n**Fontes consultadas:**\n- " + "\n- ".join(fontes)
